...
- **D5 — One hook source at a time.** Plugin hooks + Plan 2's `~/.claude/settings.json` bus group both firing = double register + double watcher-launch directive. README documents picking one; `install-hooks.sh` prints a preferred-plugin note. (Double-fire protection is documentation-only by design; no runtime guard.)
